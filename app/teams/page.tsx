...
export default async function TeamsPage() {
  ...
  const allTeams = await db.query.teams.findMany({
    ...
    with: {
      organisation: true,
      positions: { where: isNull(teams.deletedAt) },
      memberships: true,
    },
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem" }}>
        <h1 className="utrecht-heading-1" style={{ margin: 0 }}>Teams</h1>
        <a href="/teams/nieuw" className="utrecht-button utrecht-button--primary-action">+ Nieuw team</a>
      </div>
              ...
              return (
                <tr key={team.id} className="utrecht-table__row">
                  <td className="utrecht-table__cell">
                    <a href={`/teams/${team.id}`} className="utrecht-link" style={{ fontWeight: 600 }}>
                      {team.name}
                    </a>
                  </td>
                  ...
                  <td className="utrecht-table__cell">
                    <a href={`/teams/${team.id}`} className="utrecht-link" style={{ marginRight: "1rem" }}>Bekijken</a>
                    <a href={`/teams/${team.id}/bewerken`} className="utrecht-link">Bewerken</a>
                  </td>
                </tr>
              );
            })}
