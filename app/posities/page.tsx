  ...
  return (
    <div>
      ...
      <form method="get" action="/posities" style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", alignItems: "flex-end", marginBottom: "1.5rem" }}>
        ...
        <Link href="/posities" className="utrecht-button utrecht-button--subtle-action">Wis filters</Link>
      </form>

      {total === 0 ? (
        <p style={{ color: "var(--rvo-color-grijs-600)" }}>
          Geen posities gevonden.{" "}
          <Link href="/posities/nieuw" className="utrecht-link">Maak de eerste positie aan →</Link>
        </p>
      ) : (
        <>
          <p style={{ fontSize: "0.875rem", color: "var(--rvo-color-grijs-600)", marginBottom: "0.75rem" }}>
            {total === 1 ? "1 positie" : `${total} posities`}
            {total > PAGE_SIZE && ` — ${from}–${to} getoond`}
          </p>
