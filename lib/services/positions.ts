...
export async function createPosition(data: z.infer<typeof PositionSchema>, actor: Actor) {
  const [row] = await db.insert(positions).values(data).returning();
  await logAudit({ actorUserId: actor.userId, entityType: "position", entityId: row.id, action: "create", after: row });
  dispatchSync("position", row.id);
  return row;
}

export async function updatePosition(id: string, data: z.infer<typeof PositionUpdateSchema>, actor: Actor) {
  ...
  await logAudit({ actorUserId: actor.userId, entityType: "position", entityId: id, action: "update", before, after });
  dispatchSync("position", id);
  return after;
}

export async function archivePosition(id: string, actor: Actor) {
  const [before] = await db.select().from(positions).where(eq(positions.id, id));
  if (!before || before.deletedAt) throw notFound();

  await db.update(positions).set({ deletedAt: new Date(), updatedAt: new Date() }).where(eq(positions.id, id));
  await logAudit({ actorUserId: actor.userId, entityType: "position", entityId: id, action: "archive", before });
  dispatchSync("position", id);
}
